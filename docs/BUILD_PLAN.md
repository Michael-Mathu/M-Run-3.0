...
| ID | Task | Files / Modules | Acceptance Criteria | Effort | Depends on | Risk |
|---|---|---|---|---|---|---|
| `SEC-3` | Default `API_BASE_URL` to HTTPS; remove `android:usesCleartextTraffic="true"` once backend TLS exists | `api_client.dart:7-10`, `AndroidManifest.xml:14` | ⚠️ **Partial.** The Dart-side default (`kApiBaseUrl` = `http://10.0.2.2:8080`) is genuinely blocked — there is no HTTPS backend anywhere in this repo to default to, and the emulator-loopback address only ever serves local dev over plain HTTP; flipping it would just break the documented local-dev flow, not fix anything. What *was* achievable without that infra: the Android cleartext policy was a blanket `cleartextTrafficPermitted="true"` for every domain (both the manifest flag and `network_security_config.xml`'s `base-config`), meaning a misconfigured `API_BASE_URL` pointed at a real host would silently allow plaintext there too. Scoped it down — cleartext is now only permitted for `10.0.2.2`/`localhost`/`127.0.0.1` via a `domain-config`; the `base-config` (everything else) now requires HTTPS. Removed the redundant manifest flag. Not build-verified (no Android build toolchain in this environment) but standard, well-documented Android platform XML. | S (2-3h) + backend TLS setup (infra, separate) | Backend TLS termination must exist first (infra, track separately) | Blocked on infra, not code — sequence with whoever owns backend deployment. |
...
| `SEC-14` | Implement iOS `requestWhenInUseAuthorization`/`requestAlwaysAuthorization` and `didChangeAuthorization`/`didFailWithError` delegates; add missing `Info.plist` usage-description keys | `MwendoGpsEnginePlugin.swift`, `ios/Runner/Info.plist` (app + example) | ⚠️ **Done, with one correction to the original finding.** Re-reading `app/ios/Runner/Info.plist` (the real shipped app, as opposed to the plugin's standalone `example/`) before touching it: it **already has** `NSLocationWhenInUseUsageDescription`, `NSLocationAlwaysAndWhenInUseUsageDescription`, and `UIBackgroundModes: [location]` — the audit's "near-certain crash risk" claim was about the plugin package's `example/ios/Runner/Info.plist` only (dev/testing scaffolding for `packages/mwendo_gps_engine` in isolation), not the real app users would install. Added the same three keys there too for consistency. The actual code gap was real, though: `MwendoGpsEnginePlugin.swift`'s `startRecording` never checked or requested authorization at all — it called `startUpdatingLocation()` and returned success unconditionally. Restructured `startRecording` to check `authorizationStatus` first: proceeds immediately if already authorized, calls `requestAlwaysAuthorization()` and defers the Flutter result via a `pendingStartResult` (mirroring the Android Kotlin plugin's existing pattern) if `.notDetermined`, and returns a `FlutterError` for `.denied`/`.restricted` instead of silently pretending to start. Added `locationManagerDidChangeAuthorization` (iOS 14+) and the pre-14 `didChangeAuthorization` delegate to resolve that pending result, plus `didFailWithError` to surface CoreLocation errors instead of swallowing them. **Not build-verified — no Xcode/macOS toolchain in this environment; needs a real device/simulator pass before trusting it in production**, per the row's own original risk note. | M (1-1.5 days) | none | Real device/simulator testing required — cannot be fully verified without a physical iOS test pass. |
