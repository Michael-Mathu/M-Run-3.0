...
| `PF-5` | Add antimeridian (±180° longitude) wraparound handling to the ENU coordinate transform | `packages/gps_pipeline/lib/src/coordinate_util.dart:25-42` | A test case with an origin/fix pair straddling ±180° longitude produces a correct short-distance ENU offset, not a ~40,000km error. | S (3-4h) | `TS-6` (test scaffolding) | Low. |
| `CQ-16` | Fix `copyWith` to preserve `smoothedSpeedMps` across gap/stationary-state transitions | `packages/gps_pipeline/lib/src/models.dart` (`copyWith`), call sites in `gap_detector.dart:24,28` and `stationary_suppressor.dart` | The new `TS-4`/`TS-5` ground-truth test (or a dedicated unit test) confirms `smoothedSpeedMps` survives a gap/stationary transition instead of resetting to `null`. | S (2-3h) | `TS-4`/`TS-5` | Low — isolated, well-tested change. |
